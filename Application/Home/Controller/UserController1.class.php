...
class UserController extends BaseController {
	...
	public function tongzhi(){
		$zhuyixianshi=M('zhuyi');
		$maxid = $zhuyixianshi->max('id');
		//var_dump($maxid);exit();
		if($maxid>=6)
		{
			$zhuyiALL[] =$zhuyixianshi->where(array('id'=>($maxid)-$i))->select()[0];
			
		}
		else($maxid<6){
			$zhuyiAll2[]=$zhuyixianshi->select();
			var_dump($zhuyiAll2[]);exit();
			
		}
		//$test=$zhuyixianshi->where(array('id'=>$maxid))->field('detail')->select()[0][detail];
		//$title=$zhuyixianshi->where(array('id'=>$maxid))->field('title')->select()[0][title];
		//var_dump($title);exit();
	...
	public function printzs(){
		...
		if($date==$time&&$score=="是"){   //考生考试日期与exam中当年考生是同一批考试
				// 传递一个 account 即可
		...
		if(substr($other_exam['time'], 0, 6) < substr($user_exam['time'], 0, 6)){
	        ...
	        $s="../../../Uploads/".$i."/1.jpg";
			$this->assign('s', $s);
			$zhengshu="GSJ".$time.$examNumber;//证书编码
	...
	//////
      ...
      $seal=new circleSeal('百家筝鸣连锁总部',75,9,24,0,0,20,0);  
            $seal->doImg();  
        }  
